        ...
        {trip.itinerary.map((item, idx) => (
          <div key={idx} className="mb-4 border rounded p-4 bg-white shadow-sm">
            <div className="flex justify-between items-center mb-2">
              ...
              />
              <button onClick={() => removeItineraryDay(idx)} className="edit-button text-red-500">Remove</button>
            </div>
            <textarea
...
<section className="trip-section">
  <div className="flex justify-between items-center mb-2">
    ...
    <button
      className="edit-button"
      onClick={() => setEditState({ ...editState, tripPhotos: !editState.tripPhotos })}
    >
    ...
    <>
      {trip.tripPhotos && trip.tripPhotos.map((photo, index) => (
        <div key={index} className="flex items-center gap-4 mt-2">
          ...
          />
          <button onClick={() => removePhotoBox(index)} className="edit-button text-red-500">-</button>
        </div>
      ))}
      <button onClick={addPhotoBox} className="edit-button mt-2 text-green-500">+ Add Photo</button>
    </>
  )}
